...
def _redact_secrets(text: str, secrets: list[str]) -> str:
    """Redact sensitive values before returning tool output."""
    out = text
...
def _kill_process_tree_win32(pid: int) -> None:
    ...
    try:
        ...
        # Best-effort kill: ignore failures (process may have exited)
        pass
...
def _execute_subprocess_sync(
    ...
    try:
        ...
        stdout_fd, stdout_path = tempfile.mkstemp(prefix="wisecore_out_")
        stderr_fd, stderr_path = tempfile.mkstemp(prefix="wisecore_err_")
...
async def execute_shell_command(
    ...
    Args:
        ...
        env (`Optional[dict]`, defaults to `None`):
            Extra environment variables merged onto the process environment.
    ...
    cli_token = exec_env.get("CLI_ACCESS_TOKEN")
    if isinstance(cli_token, str) and cli_token.strip():
        redact_values.append(cli_token.strip())
    ...
    try:
        ...
        stdout_str = _redact_secrets(stdout_str, redact_values)
        stderr_str = _redact_secrets(stderr_str, redact_values)
        ...
        err = _redact_secrets(str(e), redact_values)
        return ToolResponse(
            content=[
                TextBlock(
                    type="text",
