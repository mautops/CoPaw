...
description: Use this skill when another agent's expertise/context is needed, or when the user explicitly asks to involve another agent. First list agents, then use wisecore agents chat for two-way communication with replies. | 当需要其他 agent 的专长/上下文，或用户明确要求调用其他 agent 时使用；先查 agent，再用 wisecore agents chat 双向通信（有回复）
metadata: { "builtin_skill_version": "1.1", "wisecore": { "emoji": "🤝" } }
---
...
```bash
wisecore agents list
```

### 2) 发起新对话（实时模式）

```bash
wisecore agents chat \
...
```bash
wisecore agents chat --background \
  --from-agent <your_agent> \
  --to-agent <target_agent> \
  --text "[Agent <your_agent> requesting] ..."
```
...
```bash
wisecore agents chat --background --task-id <task_id>
```
...
| 任务类型 | 使用模式 | 命令 |
|---------|---------|------|
| 简单快速查询 | 实时模式 | `wisecore agents chat` |
| 复杂任务（数据分析、批量处理等） | 后台模式 | `wisecore agents chat --background` |
...
2. wisecore agents list
3. wisecore agents chat 发起对话
4. 从输出中记录 [SESSION: ...]
...
2. wisecore agents list
3. wisecore agents chat --background 提交任务
4. 从输出中记录 [TASK_ID: ...]
5. 继续处理其他工作
...
```bash
wisecore chats list --agent-id <your_agent>
```
...
Check status with:
  wisecore agents chat --background --task-id 20802ea3-...
```
...
# 方式 2：如果必须等待，使用合理间隔
sleep 30 && wisecore agents chat --background \
  --task-id 20802ea3-832d-4fb4-86f0-666ad79fcc80
```
...
# 3. 在适当时机查询结果
# （比如处理完当前任务后，或用户询问进度时）
wisecore agents chat --background --task-id <id>
```

**方式 2：定时轮询**（如果必须等待）
```bash
# 递增间隔，先快后慢
sleep 10 && wisecore agents chat --background --task-id <id>
sleep 20 && wisecore agents chat --background --task-id <id>
sleep 30 && wisecore agents chat --background --task-id <id>
```
...
```bash
wisecore agents -h
wisecore agents list -h
wisecore agents chat -h
```