...
if TYPE_CHECKING:
    from wisecore.config.config import AgentProfileConfig
...
def get_token_counter(
    ...
    Args:
        ...
        use_estimate: If True (default), returns a EstimateTokenCounter
    ...
    if use_estimate:
        ...
        if config_key not in _token_counter_cache:
            ...
            logger.debug(
                "Token counter created with "
                "model=%s, mirror=%s, divisor=%s",
                cc.token_count_model,
                cc.token_count_use_mirror,
