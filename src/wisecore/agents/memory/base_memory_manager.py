    ...
    def add_async_summary_task(self, messages: list[Msg], **kwargs):
        ...
        for task in self.summary_tasks:
            if task.done():
                ...
                exc = task.exception()
                if exc is not None:
                    logger.error("Summary task failed: %s", exc)
                else:
                    result = task.result()
                    logger.info("Summary task completed: %s", result)
            else:
            ...
            if task.done():
                ...
                if task.cancelled():
                    ...
                    if exc is not None:
                        ...
                        logger.info("Summary task completed: %s", task_result)
                ...
                try:
                    ...
                    logger.exception("Summary task failed: %s", e)
