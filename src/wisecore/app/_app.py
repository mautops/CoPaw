...
from ..utils.logging import setup_logger, add_wisecore_file_handler
from .auth import AccessTokenUserMiddleware
...
async def lifespan(
    ...
    add_wisecore_file_handler(WORKING_DIR / "wisecore.log")
...
# After token resolution: bind wisecore_subject -> primary agent_id
app.add_middleware(UserPrimaryAgentMiddleware)
# Outermost before CORS: Keycloak token introspection for all /api/*
app.add_middleware(AccessTokenUserMiddleware)

# Apply CORS middleware
# Default to allowing common dev origins; override with CORS_ORIGINS
_cors_origins = CORS_ORIGINS.strip() if CORS_ORIGINS else ""
if _cors_origins == "*":
    _allow_origins = ["*"]
elif _cors_origins:
...
# Console static dir: env override only (next-console is the primary UI).
_CONSOLE_STATIC_ENV = "CONSOLE_STATIC_DIR"


def _resolve_console_static_dir() -> Optional[str]:
    """Return console static dir if configured via env var.

    The primary web UI is next-console (separate Next.js app).
    This fallback exists for bundled SPA deployments
    via CONSOLE_STATIC_DIR.
    """
...
_CONSOLE_INDEX = (
    Path(_CONSOLE_STATIC_DIR) / "index.html"
...
def read_root():
    ...
    return {
        "message": (
            "The web UI is not bundled with this API process. "
            "Deploy or run the `next-console` app (see `next-console/`) "
            "and open it in the browser; set `API_URL` to this "
            "server's base URL."
        ),
    }
...
# Console static files and SPA fallback (optional bundled UI)
# Register these AFTER API routes to ensure proper routing priority
if _CONSOLE_STATIC_DIR and Path(_CONSOLE_STATIC_DIR).is_dir():
    ...
    @app.get("/wisecore-symbol.svg")
    def _console_icon():
        f = _console_path / "wisecore-symbol.svg"
        if f.is_file():
            return FileResponse(f, media_type="image/svg+xml")
        raise HTTPException(status_code=404, detail="Not Found")

    @app.get("/wisecore-dark.png")
    def _console_dark_icon():
        f = _console_path / "wisecore-dark.png"
        if f.is_file():
            return FileResponse(f, media_type="image/png")
        raise HTTPException(status_code=404, detail="Not Found")
