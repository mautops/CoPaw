...
class VoiceChannelConfig(BaseChannelConfig):
    ...
    language: str = "en-US"
    welcome_greeting: str = "Hi! This is your AI assistant. How can I help you?"
...
class WeixinConfig(BaseChannelConfig):
    ...
    bot_token_file: Path to persist/load the bot_token
                    (default ~/.wisecore/weixin_bot_token).
...
def load_agent_config(agent_id: str) -> AgentProfileConfig:
    ...
    # Normalize legacy ~/.wisecore-bound paths to current WORKING_DIR.
    # This keeps WORKING_DIR effective even if existing agent.json
    # contains older hard-coded paths like "~/.wisecore/media".
    try:
        from .utils import _normalize_working_dir_bound_paths

        data = _normalize_working_dir_bound_paths(data)
    except Exception as e:
        logger.debug("Failed to normalize working dir bound paths: %s", e)
        pass
...
def migrate_legacy_config_to_multi_agent() -> bool:
    ...
    # When WORKING_DIR is customized, historical data may still exist
    # under "~/.wisecore".
    old_workspace = Path("~/.wisecore").expanduser().resolve()
