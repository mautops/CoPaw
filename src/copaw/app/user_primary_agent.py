...
JWT, ``request.state.copaw_subject`` is the mailbox local-part (JWT ``sub``),
...
def user_agent_binding_enabled() -> bool:
    """When false, skip auto-binding (env ``COPAW_USER_AGENT_BINDING``)."""
    v = os.environ.get("COPAW_USER_AGENT_BINDING", "0").strip().lower()
...
class UserPrimaryAgentMiddleware(BaseHTTPMiddleware):
    ...
    async def dispatch(
        ...
        subject = getattr(request.state, "copaw_subject", None)
        if not subject:
            return await call_next(request)
